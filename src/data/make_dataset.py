...
def ChainCheck(df):
    '''
    Purpose:
        Given multiple mutations, ensures they all occur within the
        same set of chains that make up either protein.
...
# Convert non-numeric 'Temperature' comments to numeric values. Default is 298
...
@click.argument('input_filepath', type=click.Path(exists=True)) # must exist
@click.argument('interim_filepath', type=click.Path()) # will create
...
# 1.1a save intermediate to interim
    skempi_final.to_csv('~/data/interim')

# 1.2 – Import Other
    # other_final = Clean_Other(input_filepath + 'other.csv')
# 1.2a – save intermediate to intermediates
